...
export default async function LeaderboardPage({
  ...
  const [leaderboard, leagues] = await Promise.all([
    getLeaderboard({ league, period, minGames, sort, q }),
    getLeagues(),
  ]);
  ...
  return (
    <>
      <PageHeader
        eyebrow={leaderboard.source}
        title="Player Contribution Leaderboard"
        description="MVP ranking for Team Kazakhstan players based on official match participation, chess points, and contribution score."
      />
      <Card className="mb-6">
        <form className="grid gap-4 md:grid-cols-[1fr_1fr_1fr_auto] md:items-end">
          ...
          <label className="text-sm text-slate-300">
            ...
            >
              ...
              {leagues.data
                .filter(
                  (item) => item.slug !== "unknown" && item.slug !== "friendly",
                )
                .map((item) => (
