...
export default async function MatchesPage({ searchParams }: { searchParams: Promise<{ official?: string; league?: string; month?: string }> }) {
  ...
  const [matches, leagues] = await Promise.all([getMatches({ official, league, month }), getLeagues()]);
      ...
      <Card className="mb-6 space-y-4">
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            <FilterLink href={filterHref("all", league, month)} active={official === "all"}>All</FilterLink>
            <FilterLink href={filterHref("official", league, month)} active={official === "official"}>Official</FilterLink>
          </div>
        </div>
        <div>
          <p className="mb-2 text-sm font-medium text-slate-300">League</p>
          <div className="flex flex-wrap gap-2">
            <FilterLink href={filterHref(official, "all", month)} active={league === "all"}>All leagues</FilterLink>
            {leagues.data.map((item) => (
              <FilterLink key={item.slug} href={filterHref(official, item.slug, month)} active={league === item.slug}>{`${item.name}${item.matchCount === 0 ? " (no data yet)" : item.matchCount == null ? "" : ` (${item.matchCount})`}`}</FilterLink>
            ))}
          </div>
        </div>
        {month ? <p className="text-sm text-slate-400">Showing matches for {month}. <Link href={filterHref(official, league)} className="text-cyan-200 hover:text-cyan-100">Clear month filter</Link></p> : null}
      </Card>
