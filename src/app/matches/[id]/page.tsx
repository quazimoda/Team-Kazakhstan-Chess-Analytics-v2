...
export default async function MatchDetailsPage({
  ...
  const [matchResult, participationResult, gameResult] = await Promise.all([
    getMatchById(id),
  ...
  return (
    <>
      <PageHeader
        eyebrow={matchResult.source}
        title={match.name}
        description="Match metadata, synced player participation rows, and Chess.com games."
      />
      <div className="mb-6">
        <Link
          href="/matches"
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-1">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            ...
            <div>
              <dt className="text-slate-400">Opponent</dt>
              <dd className="text-white">
                {readableOpponentName(match.opponent, match.name)}
              </dd>
            </div>
            <div>
            ...
            <div>
              ...
              <dd className="text-white">{match.boardCount ?? "—"}</dd>
            </div>
            <div>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            {opponentUrl ? (
              <Link
                href={opponentUrl}
                className="rounded-2xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-cyan-100 transition hover:bg-white/10"
              >
                Open opponent
              </Link>
            ) : null}
        ...
        <Card className="overflow-x-auto lg:col-span-2">
          ...
          {participationResult.data.length === 0 ? (
            <p className="mt-4 rounded-2xl border border-yellow-300/20 bg-yellow-300/10 p-4 text-sm text-yellow-100">
              Details have not been synced for this match yet. Run Admin → Sync
              Match Details to populate board/player rows.
            </p>
          ) : (
            <table className="mt-4 w-full min-w-[720px] text-left text-sm">
              ...
              <tbody>
                {participationResult.data.map((row) => (
                  <tr
                    key={row.playerId}
                    className="border-b border-white/5 text-slate-200 last:border-0"
                  >
                    <td className="py-4">{row.boardNumber ?? "—"}</td>
      ...
      <Card className="mt-6 overflow-x-auto">
        ...
        {gameResult.data.length === 0 ? (
          <p className="mt-4 text-sm text-slate-400">
            No games are stored for this match yet.
          </p>
        ) : (
          <table className="mt-4 w-full min-w-[720px] text-left text-sm">
            <thead className="text-slate-400">
              <tr className="border-b border-white/10">
                <th className="py-3">White</th>
                <th>Black</th>
                <th>Result</th>
                <th>Time class</th>
                <th>Rated</th>
            ...
            <tbody>
              {gameResult.data.map((game) => (
                <tr
                  key={game.id}
                  className="border-b border-white/5 text-slate-200 last:border-0"
                >
                  <td className="py-4">{game.whiteUsername ?? "—"}</td>
                  ...
                  <td>{formatDateTime(game.endTime)}</td>
                </tr>
              ))}
            </tbody>
          </table>
